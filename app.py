...
KT_SIMULTANEITY_NOTE = (
    "* **同时使用率 Kt 在模型中按 ≤1 理解**（同一时间启用的桩功率占比不超过 100%）。"
    "仅当 **Kt < 1** 时存在超需风险，建议配置**桩侧限功率**或**本地微网控制器**；"
    "**Kt = 1** 时按满载同时启用理解，**无需**微网控制器。"
)
KT_MICROGRID_REQUIRED_NOTE = (
    "**Kt < 1**：存在超需风险，建议配置**桩侧限功率**或**本地微网控制器**"
    "（**第四部分 · 微网控制器** 填写设备与调试费）。"
)
KT_MICROGRID_NOT_REQUIRED_NOTE = (
    "**Kt = 1**：按模型可同时满载，**无需**配置微网控制器；下述费用可填 **0**。"
)
...
def _html_kt_microgrid_alert(kt: float | None) -> None:
    ...
    if kt_needs_microgrid_controller(kt):
        st.markdown(
            '<div class="kt-microgrid-banner">'
            '<div class="kt-microgrid-banner-title">Kt &lt; 1 · 建议配置微网控制器</div>'
            "<p><strong>Kt &lt; 1</strong>：存在超需风险，建议配置"
            "<strong>桩侧限功率</strong>或<strong>本地微网控制器</strong>"
            "（<strong>第四部分 · 微网控制器</strong> 填写设备与调试费）。</p>"
            "</div>",
        ...
        st.markdown(
            ...
            "<strong>Kt = 1</strong>：按模型可同时满载，<strong>无需</strong>配置微网控制器；"
            "下述费用可填 <strong>0</strong>。</p></div>",
            unsafe_allow_html=True,
        )
...
def _html_construction_subsection_title(text: str) -> None:
    """第四部分各工程子标题（字号略小于五步大标题）。"""
    st.markdown(
        f'<div class="construction-subsection-title">{text}</div>',
        unsafe_allow_html=True,
    )
...
def _html_construction_section_total(total_yuan: float, section_name: str) -> None:
    ...
    st.markdown(
        f'<p class="muted-hint" style="font-weight:700;color:#0f172a;margin-top:0.75rem;">'
...
def _html_len_price_header() -> None:
    ...
    with h1:
        st.caption("**长度 / 数量**")
    with h2:
        st.caption("**单价(元 或 元/m)**")


def _html_op_param_box(title: str) -> None:
    """第五部分：充电桩/储能运营参数分区框。"""
    st.markdown(
        f'<div class="op-param-box"><div class="op-param-box-title">{title}</div>',
        unsafe_allow_html=True,
    )
...
def render_charger_engineering_inputs(
    ...
    *,
    ...
    with st.expander("低压交流电力电缆", expanded=True):
...
def render_storage_engineering_inputs(
    ...
    if not storage_installed:
        st.info(STORAGE_ENG_CAPEX_EXCLUDED_NOTE)
    else:
        st.caption(
...
**2. 峰谷套利+充电**（储能给桩供电，假设充电在高峰）  
- 日总充电量 = 各枪日充电量之和；储能日可放 `E₀=装机×日循环`；供桩 `E=min(E₀,日总充电量)`  
- **日毛利** = 日总充电量×服务费 − (日总充电量−E)×**峰电价** − (E÷效率)×**谷电价**  
    ...
    *,
    ...
    with c1:
        kt_level = st.selectbox(
            ...
            help="1=满载同时（无需微网控制器）；高0.7-0.9；低0.6-0.7；中间值0.7",
            key=sk + "b_kt_level",
        )
    ...
    with c5:
        st.markdown(f'<p class="muted-hint" style="margin-top:1.6rem;">规划用 **Kt = {kt:.2f}**</p>', unsafe_allow_html=True)

    if kt_needs_microgrid_controller(kt):
        st.markdown(
            f'<p class="muted-hint" style="color:#b45309;">{KT_MICROGRID_REQUIRED_NOTE}</p>',
            unsafe_allow_html=True,
        )
    else:
    ...
    st.markdown(
        ...
        f"充电枪合计约 **{guns_60 + guns_120 + guns_480}** 枪"
    )
    if spots_used > parking_spots:
        st.warning(f"占用车位 {spots_used} 个，超过场站 {parking_spots} 个，请减桩或增加车位。")

    st.markdown("**配电校验（按上式系数）**")
    st.markdown(
        f"- 装机总功率 **{total_kw:.0f} kW** → 需用变压器约 **{req_kva:.1f} kVA**\n"
        f"- 第一步剩余可用 **{kva_remaining:.1f} kVA**"
        + (
    ...
    if need_upgrade:
        if rec_kva is not None:
            st.info(
                ...
                "请在下方确认目标容量与增容单价；若第一步为「不增容」请改选原址换大/高压申请。"
            )
        else:
            st.warning(
    ...
    _html_section_title("工商业储能（本模式配置）")
    ...
    if install_storage:
        ...
        st.caption(
            ...
            f"合计 **{storage_sets * STORAGE_KWH_PER_SET_DEFAULT:.0f} kWh** / "
...
def render_charger_capacity_planner(
    *,
    ...
    _html_section_title("分体式液冷 480kW（选填）")
    st.caption(
    ...
    _html_section_title("60 / 120 / 480 最优搭配")
    ...
    st.markdown(
        f'<p class="muted-hint">按系数测算：允许桩总功率约 <strong>{max_kw:.0f} kW</strong>。</p>',
        unsafe_allow_html=True,
    )
...
def _html_section_title(text: str) -> None:
    """表单/结果分区大标题（加粗放大）。"""
    st.markdown(f'<div class="section-block-title">{text}</div>', unsafe_allow_html=True)


def _html_part_divider() -> None:
    """大段之间的分隔线（如充电桩 ↔ 储能）。"""
    st.markdown('<hr class="section-part-divider" />', unsafe_allow_html=True)
...
def _builtin_defaults_raw() -> dict[str, Any]:
    ...
    return {
        ...
        "usage": USAGE_HYBRID,
        "n_pile_60": 0,
        "price_pile_60_wan": 0.0,
        "n_pile_120": 0,
        "price_pile_120_wan": 0.0,
        "n_ultra_480": 0,
...
<style>
  ...
  .kpi-sub { color: #94a3b8; font-size: 0.78rem; margin-top: 4px; line-height: 1.35; }
  .page-hero {
    padding: 8px 0 16px;
...
REVENUE_METRIC_HINTS: dict[str, str] = {
    ...
    "储充一体经营毛利(年化)": (
        "日毛利×运营天数；日毛利=充电量×服务费−峰段网购电−谷段储能充电。"
        "网购=（日充电−储能供桩量）×峰价；储能充电=供桩量÷效率×谷价。"
    ),
...
def _html_metric_hint(key: str) -> None:
    ...
    if hint:
        st.caption(f'<span class="metric-hint">{hint}</span>', unsafe_allow_html=True)
...
def _annual_revenue_by_usage(
    ...
    *,
    ...
    if inp.usage == USAGE_HYBRID:
        ...
        grid_kwh_day = max(0.0, kwh_charge_day - e_to_pile)
        storage_charge_day = e_to_pile / eta if e_to_pile > 0 else 0.0
        daily_gross = (
            kwh_charge_day * fee
            - grid_kwh_day * peak
            - storage_charge_day * valley
        )
        # 日毛利已内含「服务费 − 峰段网购 − 谷段储能充电」，不再单独加毛服务费
...
def _storage_revenue_calc_lines(
    ...
    *,
    ...
    if usage_s == USAGE_HYBRID:
        ...
        grid_kwh = max(0.0, kwh_day - e_pile)
        storage_charge_day = e_pile / eta if e_pile > 0 and eta > 0 else 0.0
        daily_gross = kwh_day * fee - grid_kwh * peak - storage_charge_day * valley
        ...
        lines.extend(
            [
                ...
                f"<strong>峰段网购</strong> = (日充电 − E) × 峰价 = {grid_kwh:.0f} × {peak:.2f} = <code>{grid_kwh * peak:,.0f}</code> 元/天",
                f"<strong>谷段储能充电</strong> = E ÷ η × 谷价 = {e_pile:.0f} ÷ {eta:.0%} × {valley:.2f}"
                f" = <code>{storage_charge_day * valley:,.0f}</code> 元/天",
                f"<strong>日毛利</strong> = 日充电×服务费 − 峰网购 − 谷储能充电"
                f" = {kwh_day:.0f}×{fee:.3f} − {grid_kwh * peak:,.0f} − {storage_charge_day * valley:,.0f}"
        ...
        lines.append("本模式下<strong>不再单独列</strong>充电服务费（已含在日毛利内）。")
        return lines
...
def render_storage_operations_preview(
    *,
    ...
    if is_hybrid and storage_kwh > 0:
        ...
        excess = max(0.0, e_max - e_pile)
        st.caption(
            f"储充一体：供桩 **{e_pile:.0f}** kWh/天 · 余电套利 **{excess:.0f}** kWh/天 · "
            f"峰谷价差 **{spread:.2f}** 元/kWh（峰 {peak:.2f} / 谷 {valley:.2f}）"
        )
    ...
    if charger_rev_yuan > 1e-6 and not is_hybrid:
        ...
        s2.caption("峰谷+充电模式下，充电服务费已含在「储充一体经营毛利」内。")
...
def compute_projection(inp: InputModel) -> dict[str, Any]:
    ...
    revenue_year = op_rev + revenue_vpp

    opex_year = capex_total * max(0.0, float(inp.opex_ratio))
    net_year = revenue_year - opex_year
...
def build_cash_flows_wan(capex_yuan: float, net_year_yuan: float, horizon_years: int) -> np.ndarray:
    """
...
def _build_detail_defaults(site_basics: dict[str, Any]) -> dict[str, Any]:
    ...
    baseline = _transformer_baseline_kva(k_existing, k_remain)
    if upgrade == "不增容":
        k_target = baseline
    elif upgrade == "原址换大":
        k_target = max(baseline, float(cfg.get("kva_target", baseline)))
    else:
        k_target = max(baseline * 1.5, float(cfg.get("kva_target", 2000.0)))
...
def render_site_basics_step() -> tuple[bool, list[str]]:
    ...
    with st.form("site_basics_form"):
        _html_section_title("第一步：确认场站基础信息")
        st.markdown(
            '<p class="muted-hint">请先确认场站条件；完成后再进入电价、充电桩与储能等详细测算。</p>',
            unsafe_allow_html=True,
        )
        city = st.text_input(
            "场站地点（城市 / 行政区 / 项目名称）",
...
def render_input_form(defs: dict[str, Any]) -> tuple[bool, InputModel | None, list[str]]:
    ...
    if plan_mode == CHARGER_CONFIG_MODE_A:
        ...
        st.caption(
            "**A 按剩余容量规划**：下表桩数默认=上节推荐，**可修改**；单价可填；提交时按配电公式校验并计 CAPEX。"
        )
        ...
        with g2:
            ...
            p120_wan = float(
                st.number_input(
                    "120kW 单价(万元/台)",
                    min_value=0.0,
                    value=float(d["price_pile_120_wan"]),
                    step=1.0,
                )
            )
        with g3:
        ...
        st.caption(
            "**B 自定义桩和储能**：桩数与储能套数见上节；此处仅填 **设备单价（万元）**。"
        )
        st.markdown(
    ...
    if plan_mode == CHARGER_CONFIG_MODE_B:
        ...
        st.caption("当前为 **不增容**：ΔkVA = 0，无需填写变压器增容单价。")
        ...
        if exp_pre > 1e-3:
            st.info(
                f"相对剩余可用需增容 **{exp_pre:.1f} kVA**；在基准 **{baseline_kva:.0f} kVA** 上扩建 → "
        ...
        k_target = float(
            st.number_input(
                "目标变压器总容量(kVA)",
                min_value=float(baseline_kva),
                value=max(float(d["kva_target"]), sug_pre, baseline_kva),
                step=50.0,
                help="CAPEX 按 max(0, 目标−基准)×单价计入总投资。",
            )
        )
        pu = float(
            st.number_input(
    ...
    _html_part_divider()
    _html_section_title("第三部分：工商业储能")
    ...
    if not install_storage:
        ...
        if is_mode_b:
            ...
            st.caption(
                f"装机 **{total_kwh:.0f} kWh** / **{sets * STORAGE_KW_PER_SET_DEFAULT:.0f} kW** · "
                f"投资粗算 **{invest_wan:.2f} 万元**"
            )
        else:
        ...
        st.markdown("##### b. 储能主要用途")
        usage = st.selectbox(
            "储能主要用途",
            options=USAGE_OPTIONS,
            index=_select_index(USAGE_OPTIONS, str(d["usage"]), USAGE_HYBRID),
            help="决定年收益公式；纯动态增容不计储能循环套利。",
        )
        is_dynamic_usage = usage == USAGE_DYNAMIC_CAPACITY

        if is_dynamic_usage:
            st.markdown("##### c. 动态增容参数")
    ...
    _html_part_divider()
    _html_section_title("第四部分：施工成本")
    st.caption(
        "分为 **充电桩工程**、**微网控制器**、**储能工程**、**相关物料**（工程块之间虚线分隔）；"
        "金额内部按「元」计，结果表以万元展示。"
    )
    plan_kt_micro = float(
    ...
    _html_section_title("第五部分：运营与测算")
    st.caption("分为 **充电桩运营** 与 **储能运营** 两个参数区；未安装工商业储能时不显示储能区。")
    ...
    with st.expander("经营指标与财务报表", expanded=False):
        st.caption(
...
def _render_capex_breakdown_grouped(res: dict[str, Any]) -> None:
    ...
    if device_rows:
        st.markdown('<p class="construction-subsection-title">设备购置与变压器</p>', unsafe_allow_html=True)
...
def _render_estimate_panel_body(inp: InputModel) -> None:
    ...
    cf_wan = build_cash_flows_wan(res["capex_total"], res["net_year"], inp.horizon_years)
...
"""
    ...
    if inp.calc_financial_statements:
        ...
        m = fin["metrics"]
        _html_section_title("财务评价指标（全投资，不含税口径）")
        ...
        c3.metric(
            "IRR (XIRR)",
            f"{irr_fin * 100:.2f}%" if irr_fin is not None and np.isfinite(irr_fin) else "—",
        )
        pi_v = m.get("PI")
...
def main() -> None:
    ...
    st.caption(
        "场站 → 充电桩 → 储能（可选）→ 施工 → 运营；点击 **确认测算** 后右侧展示结果。"
        " 储能三种收益模型见 MEMORY.md。"
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col_left:
        if not st.session_state.site_basics_confirmed:
            ...
            if st.button("重新填写场站信息", type="secondary"):
                ...
                st.session_state.confirmed_inp = None
                st.rerun()
            _html_section_title("参数录入")
            st.markdown(
                '<p class="muted-hint">桩数、储能套数等数量类默认从 0 起填；'
                "电价与单价类仍可参考底部 <code>cost_defaults.json</code>。</p>",
                unsafe_allow_html=True,
            )
