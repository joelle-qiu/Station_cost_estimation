...
USAGE_DECISION_MODE1 = "模式一：峰谷套利优先"
USAGE_DECISION_MODE2 = "模式二：动态增容优先"
USAGE_DECISION_MODE1_LEGACY = "模式一：峰谷套利"
USAGE_DECISION_MODE2_LEGACY = "模式二：纯动态增容"
USAGE_DECISION_OPTIONS = [USAGE_DECISION_MODE1, USAGE_DECISION_MODE2]
MODE1_PEAK_CHARGE_SHARE_DEFAULT = 0.65
...
# 120kW 双枪：1 台按 2 枪计日充电量
PILE_120_GUNS_PER_UNIT = 2
...
# （字段后缀, 界面简称, 默认单价 元/m, 用途说明）
CHARGER_CABLE_SPECS: list[tuple[str, str, float, str]] = [
    (
        "cable_ac_main",
        "ZRC-YJV-0.6/1kV-3×240mm²+2×120mm²",
        115.0,
        "充电桩主柜→物业侧开关/箱变",
    ),
    (
        "cable_yjvr150",
        "ZRC-YJVR-0.6/1kV-150mm²",
        1030.0,
        "电源电缆4根：直流电源，2正、2负",
    ),
    (
        "cable_yjvr70",
        "ZRC-YJVR-0.6/1kV-70mm²",
        70.0,
        "PE",
    ),
]

CHARGER_PIPE_SPECS: list[tuple[str, str, float, str]] = [
    ("pipe50", "热镀锌钢管 φ50", 3.0, "电缆沟到各设备之间穿管"),
    ("pipe100", "热镀锌钢管 φ100", 60.0, "电缆沟到各设备之间穿管"),
]

CHARGER_PIPE_DEFAULT_LEN_M = 5.0

LAY_MODE_TRAY = "桥架"
LAY_MODE_TRENCH = "电缆沟"
LAY_MODE_DIRECT = "直埋"
CHARGER_LAY_MODE_OPTIONS = [LAY_MODE_TRAY, LAY_MODE_TRENCH, LAY_MODE_DIRECT]

CHARGER_ENG_LEN_KEYS = [f"chg_eng_{s[0]}_len_m" for s in CHARGER_CABLE_SPECS] + [
    "chg_eng_lay_len_m",
]

CHARGER_PIPE_LEN_KEYS = [f"chg_eng_{s[0]}_len_m" for s in CHARGER_PIPE_SPECS]
...
CHARGER_ENG_BUILTIN_DEFAULTS: dict[str, Any] = {
    ...
    "chg_eng_cable_ac_main_unit_yuan": 115.0,
    "chg_eng_cable_yjvr150_unit_yuan": 1030.0,
    "chg_eng_cable_yjvr70_unit_yuan": 70.0,
    "chg_eng_pipe50_unit_yuan": 3.0,
    "chg_eng_pipe100_unit_yuan": 60.0,
    "chg_eng_grounding_flat_yuan": 200.0,
    "chg_eng_fire_seal_yuan": 2900.0,
    "chg_eng_lay_mode": LAY_MODE_TRENCH,
    "chg_eng_lay_tray_unit_yuan": 240.0,
    "chg_eng_lay_trench_unit_yuan": 42.0,
...
class ChargerEngineeringInputs:
    ...
    cable_ac_main_len_m: float
    cable_ac_main_unit_yuan: float
    cable_yjvr150_len_m: float
    cable_yjvr150_unit_yuan: float
    cable_yjvr70_len_m: float
    cable_yjvr70_unit_yuan: float
    pipe50_len_m: float
    ...
    lay_mode: str
    lay_len_m: float
    lay_tray_unit_yuan: float
    lay_trench_unit_yuan: float
...
def _migrate_charger_eng_cfg(out: dict[str, Any]) -> None:
    """旧版 chg_eng 电缆/敷设键 → 新规格（兼容已保存默认配置与方案快照）。"""
    if "chg_eng_cable_ac_main_len_m" not in out:
        out["chg_eng_cable_ac_main_len_m"] = float(
            out.pop("chg_eng_cable_yjlhv300_len_m", 0) or 0
        )
        out["chg_eng_cable_ac_main_unit_yuan"] = float(
            out.get("chg_eng_cable_ac_main_unit_yuan")
            or out.pop("chg_eng_cable_yjlhv300_unit_yuan", 115.0)
            or 115.0
        )
    if "chg_eng_cable_yjvr150_len_m" not in out:
        out["chg_eng_cable_yjvr150_len_m"] = float(
            out.pop("chg_eng_cable_yjv240_len_m", 0)
            or out.pop("chg_eng_cable_yjv185_len_m", 0)
            or 0
        )
        out["chg_eng_cable_yjvr150_unit_yuan"] = float(
            out.get("chg_eng_cable_yjvr150_unit_yuan")
            or out.pop("chg_eng_cable_yjv240_unit_yuan", 1030.0)
            or 1030.0
        )
    if "chg_eng_cable_yjvr70_len_m" not in out:
        out["chg_eng_cable_yjvr70_len_m"] = float(
            out.pop("chg_eng_cable_yjv70_len_m", 0) or 0
        )
        out["chg_eng_cable_yjvr70_unit_yuan"] = float(
            out.get("chg_eng_cable_yjvr70_unit_yuan")
            or out.pop("chg_eng_cable_yjv70_unit_yuan", 70.0)
            or 70.0
        )
    for obsolete in (
        "chg_eng_cable_yjlhv240_len_m",
        "chg_eng_cable_yjlhv240_unit_yuan",
        "chg_eng_cable_yjv185_len_m",
        "chg_eng_cable_yjv185_unit_yuan",
        "chg_eng_lay_slope_len_m",
        "chg_eng_lay_slope_unit_yuan",
        "chg_eng_dc_material",
        "chg_eng_dc_copper_len_m",
        "chg_eng_dc_copper_unit_yuan",
        "chg_eng_dc_aluminum_len_m",
        "chg_eng_dc_aluminum_unit_yuan",
    ):
        out.pop(obsolete, None)
    if "chg_eng_lay_mode" not in out or not str(out.get("chg_eng_lay_mode", "")).strip():
        out["chg_eng_lay_mode"] = LAY_MODE_TRENCH
    if "chg_eng_lay_len_m" not in out:
        out["chg_eng_lay_len_m"] = float(
            out.pop("chg_eng_lay_direct_len_m", 0)
            or out.pop("chg_eng_lay_slope_len_m", 0)
            or 0
        )
    if "chg_eng_lay_tray_unit_yuan" not in out:
        out["chg_eng_lay_tray_unit_yuan"] = float(
            out.pop("chg_eng_lay_slope_unit_yuan", 240.0) or 240.0
        )
    for pk in CHARGER_PIPE_LEN_KEYS:
        if float(out.get(pk, 0) or 0) <= 0:
            out[pk] = CHARGER_PIPE_DEFAULT_LEN_M


def _charger_eng_lay_unit_yuan(cfg: dict[str, Any], mode: str) -> float:
    """按敷设方式取单价(元/m)。"""
    m = str(mode).strip()
    if m == LAY_MODE_TRAY:
        return float(cfg.get("chg_eng_lay_tray_unit_yuan", 240.0))
    if m == LAY_MODE_DIRECT:
        return float(cfg.get("chg_eng_lay_direct_unit_yuan", 42.0))
    return float(cfg.get("chg_eng_lay_trench_unit_yuan", 42.0))


def _charger_eng_defaults_merged(cfg: dict[str, Any], farthest_m: float) -> dict[str, Any]:
    """合并内置/配置文件默认值；电缆/敷设长度默认 = 路径长度，钢管默认 5m。"""
    out = {**CHARGER_ENG_BUILTIN_DEFAULTS}
    for k, v in cfg.items():
        if k.startswith("chg_eng_"):
            out[k] = v
    _migrate_charger_eng_cfg(out)
...
def charger_engineering_from_dict(d: dict[str, Any]) -> ChargerEngineeringInputs:
    ...
    return ChargerEngineeringInputs(
        ...
        cable_ac_main_len_m=float(c["chg_eng_cable_ac_main_len_m"]),
        cable_ac_main_unit_yuan=float(c["chg_eng_cable_ac_main_unit_yuan"]),
        cable_yjvr150_len_m=float(c["chg_eng_cable_yjvr150_len_m"]),
        cable_yjvr150_unit_yuan=float(c["chg_eng_cable_yjvr150_unit_yuan"]),
        cable_yjvr70_len_m=float(c["chg_eng_cable_yjvr70_len_m"]),
        cable_yjvr70_unit_yuan=float(c["chg_eng_cable_yjvr70_unit_yuan"]),
        ...
        lay_mode=str(c.get("chg_eng_lay_mode", LAY_MODE_TRENCH)),
        lay_len_m=float(c["chg_eng_lay_len_m"]),
        lay_tray_unit_yuan=float(c["chg_eng_lay_tray_unit_yuan"]),
        lay_trench_unit_yuan=float(c["chg_eng_lay_trench_unit_yuan"]),
...
def compute_charger_engineering_capex(ce: ChargerEngineeringInputs) -> tuple[float, dict[str, float]]:
    ...
    for suffix, label, _, _hint in CHARGER_CABLE_SPECS:
        ln = max(0.0, float(getattr(ce, f"{suffix}_len_m")))
        up = max(0.0, float(getattr(ce, f"{suffix}_unit_yuan")))
        bd[f"电力电缆·{label}"] = ln * up
    for suffix, label, _, _hint in CHARGER_PIPE_SPECS:
        ln = max(0.0, float(getattr(ce, f"{suffix}_len_m")))
        up = max(0.0, float(getattr(ce, f"{suffix}_unit_yuan")))
        bd[label] = ln * up
    ...
    lay_mode = str(ce.lay_mode).strip() or LAY_MODE_TRENCH
    lay_up = _charger_eng_lay_unit_yuan(
        {
            "chg_eng_lay_tray_unit_yuan": ce.lay_tray_unit_yuan,
            "chg_eng_lay_trench_unit_yuan": ce.lay_trench_unit_yuan,
            "chg_eng_lay_direct_unit_yuan": ce.lay_direct_unit_yuan,
        },
        lay_mode,
    )
    bd[f"敷设·{lay_mode}"] = max(0.0, ce.lay_len_m) * max(0.0, lay_up)
...
def render_charger_engineering_inputs(
    ...
    *,
    ...
    st.caption(
        ...
        f"（第一步 {farthest_m:.0f} m）；**电力电缆与敷设**长度初值同此；"
        f"**热镀锌钢管**默认 **{CHARGER_PIPE_DEFAULT_LEN_M:.0f} m**（电缆沟至各设备穿管），可改。"
    )
    ...
    st.caption("以下三项：电力电缆 → 热镀锌钢管 → 电缆敷设（方式三选一）。")

    with st.expander("① 电力电缆", expanded=False):
        _html_len_price_header()
        for suffix, label, _def_unit, hint in CHARGER_CABLE_SPECS:
            ...
            st.caption(f"用途：{hint}")

    with st.expander("② 热镀锌钢管（电缆沟至各设备穿管）", expanded=False):
        _html_len_price_header()
        pipe_vals: dict[str, tuple[float, float]] = {}
        for suffix, label, _def_unit, hint in CHARGER_PIPE_SPECS:
            c_len, c_unit = st.columns(2)
            lk = f"chg_eng_{suffix}_len_m"
            uk = f"chg_eng_{suffix}_unit_yuan"
            with c_len:
                ln = float(
                    st.number_input(
                        ...
                        step=0.5,
                        key=lk,
                        help=f"{hint}；默认 {CHARGER_PIPE_DEFAULT_LEN_M:.0f} m",
                    )
                )
            with c_unit:
                up = float(
    ...
    with st.expander("③ 电缆敷设", expanded=False):
        lay_mode = st.radio(
            "敷设方式（三选一）",
            options=CHARGER_LAY_MODE_OPTIONS,
            index=_select_index(
                CHARGER_LAY_MODE_OPTIONS,
                str(cfg.get("chg_eng_lay_mode", LAY_MODE_TRENCH)),
                LAY_MODE_TRENCH,
            ),
            horizontal=True,
            key="chg_eng_lay_mode",
        )
        cfg["chg_eng_lay_mode"] = lay_mode
        st.caption(
            f"敷设长度默认与路径一致（**{route:.1f} m**）；仅选中方式的单价参与合计。"
        )
        _html_len_price_header()
        ...
        with lay1:
            lay_len = float(
                st.number_input(
                    "敷设长度(m)",
                    min_value=0.0,
                    value=float(cfg["chg_eng_lay_len_m"]),
                    step=1.0,
                    key="chg_eng_lay_len_m",
                )
            )
        with lay2:
            lay_unit = float(
                st.number_input(
                    f"{lay_mode} 单价(元/m)",
                    min_value=0.0,
                    value=_charger_eng_lay_unit_yuan(cfg, lay_mode),
                    step=1.0 if lay_mode != LAY_MODE_TRAY else 5.0,
                    key=f"chg_eng_lay_unit_{lay_mode}",
                )
            )
        if lay_mode == LAY_MODE_TRAY:
            cfg["chg_eng_lay_tray_unit_yuan"] = lay_unit
        elif lay_mode == LAY_MODE_DIRECT:
            cfg["chg_eng_lay_direct_unit_yuan"] = lay_unit
        else:
            cfg["chg_eng_lay_trench_unit_yuan"] = lay_unit
        cfg["chg_eng_lay_len_m"] = lay_len
    ...
    ce = ChargerEngineeringInputs(
        ...
        cable_ac_main_len_m=float(cfg["chg_eng_cable_ac_main_len_m"]),
        cable_ac_main_unit_yuan=float(cfg["chg_eng_cable_ac_main_unit_yuan"]),
        cable_yjvr150_len_m=float(cfg["chg_eng_cable_yjvr150_len_m"]),
        cable_yjvr150_unit_yuan=float(cfg["chg_eng_cable_yjvr150_unit_yuan"]),
        cable_yjvr70_len_m=float(cfg["chg_eng_cable_yjvr70_len_m"]),
        cable_yjvr70_unit_yuan=float(cfg["chg_eng_cable_yjvr70_unit_yuan"]),
        ...
        lay_mode=str(cfg.get("chg_eng_lay_mode", LAY_MODE_TRENCH)),
        lay_len_m=float(cfg["chg_eng_lay_len_m"]),
        lay_tray_unit_yuan=float(cfg["chg_eng_lay_tray_unit_yuan"]),
        lay_trench_unit_yuan=float(cfg["chg_eng_lay_trench_unit_yuan"]),
        lay_direct_unit_yuan=float(cfg["chg_eng_lay_direct_unit_yuan"]),
...
def decision_mode_to_usage(mode: str) -> str:
    """新决策文案映射到内部收益口径枚举。"""
    m = str(mode).strip()
    if m in (USAGE_DECISION_MODE2, USAGE_DECISION_MODE2_LEGACY):
        return USAGE_DYNAMIC_CAPACITY
    return USAGE_PEAK_VALLEY_ONLY


def usage_to_decision_mode(usage: str) -> str:
    """内部收益口径反映射到新决策文案。"""
    if str(usage).strip() == USAGE_DYNAMIC_CAPACITY:
        return USAGE_DECISION_MODE2
    return USAGE_DECISION_MODE1


def _effective_peak_reduction_kw(peak_reduction_kw: float, storage_sets: int) -> float:
    """削峰申报值与储能装机功率取大，用于动态增容覆盖校核。"""
    storage_kw = max(0, int(storage_sets)) * STORAGE_KW_PER_SET_DEFAULT
    return max(float(peak_reduction_kw), float(storage_kw))


def render_storage_only_vs_transformer_advisory(
    *,
    baseline_kva: float,
    need_target_kva: float,
    tf_upgrade_cost_wan: float,
    storage_cost_wan: float,
    storage_covers: bool,
    has_tf_expansion_planned: bool,
) -> None:
    """
    储能可单独覆盖缺口时：对比变压器增容与储能投资，引导仅储能或仅增容二选一。
    作者: jiali.qiu
    """
    if not storage_covers:
        return
    st.success(
        "储能可单独覆盖超容缺口，请评估是否**仅保留储能方案**（动态增容优先、不计变压器增容 CAPEX）。"
    )
    st.info(
        f"**投资粗算对比**：仅变压器增容（增至建议 **{need_target_kva:.0f} kVA**）约 "
        f"**{tf_upgrade_cost_wan:.2f} 万元**；当前储能设备约 **{storage_cost_wan:.2f} 万元**。"
    )
    if has_tf_expansion_planned:
        st.markdown(
            f"若采用**仅储能**：请返回 **第二步 · 配电增容**，将 **目标变压器总容量** 调回基准 "
            f"**{baseline_kva:.0f} kVA**（ΔkVA=0，不再计增容投资）。"
        )
    else:
        st.markdown(
            f"当前 **目标变压器总容量** 为基准 **{baseline_kva:.0f} kVA**（未选增容），"
            f"属**仅储能补偿**方案；第三部分运营模式限定为 **{USAGE_DECISION_MODE2}**。"
        )
    st.caption(
        "若更倾向**变压器增容**而非储能：请在第二步将 **是否安装工商业储能** 选为 "
        "「不安装工商业储能」，并填写目标总容量完成增容。"
    )
...
PARAMETER_DEF_ROWS: list[tuple[str, str, str, str]] = [
    ...
    ("运营与测算", "是否接入其他负荷(是/否)", "has_other_load", "str"),
    ("运营与测算", "其他负荷日耗电量(kWh)", "other_load_kwh_day", "float"),
    ("运营与测算", "模式一·峰时充电占比(0-1)", "peak_charge_share_mode1", "float"),
...
def _builtin_defaults_raw() -> dict[str, Any]:
    ...
    return {
        ...
        **STORAGE_ENG_BUILTIN_DEFAULTS,
        ...
        "has_other_load": "否",
        "other_load_kwh_day": 0.0,
        "peak_charge_share_mode1": MODE1_PEAK_CHARGE_SHARE_DEFAULT,
...
def _sanitize_defaults_bundle(d: dict[str, Any]) -> dict[str, Any]:
    ...
    if out.get("has_other_load") not in HP_OPTIONS:
        out["has_other_load"] = "否"
    ...
    for k, v in merged.items():
        if k.startswith("chg_eng_") or k.startswith("sto_eng_"):
            out[k] = v
    _migrate_charger_eng_cfg(out)
...
def load_site_cost_defaults() -> dict[str, Any]:
    ...
    try:
        ...
        merged.update(
            {k: v for k, v in data.items() if k.startswith(("chg_eng_", "sto_eng_"))}
        )
...
def save_site_cost_defaults(new_values: dict[str, Any]) -> None:
    ...
    for k in sorted(new_values):
        if k.startswith(("chg_eng_", "sto_eng_")) and k not in ordered:
            ordered[k] = new_values[k]
...
<style>
  ...
  /* 右侧结果区 metric 标签允许换行，避免 480kW / 储能 等被截断 */
  div[data-testid="stHorizontalBlock"]:nth-of-type(1) > div[data-testid="column"]:last-child [data-testid="stMetricLabel"] p {
  ...
  div[data-testid="stHorizontalBlock"]:nth-of-type(1) > div[data-testid="column"]:last-child [data-testid="stMetricValue"] {
...
class InputModel:
    ...
    has_other_load: str
    other_load_kwh_day: float
    peak_charge_share_mode1: float
...
REVENUE_METRIC_HINTS: dict[str, str] = {
    ...
    "储能峰谷套利(峰时消纳)": "模式一：按峰时充电占比估算的峰时可消纳电量 × (峰价−谷价/效率) × 天数。",
    "储能峰谷套利(平时消纳)": "模式一：峰时未消纳且可被平时负荷吸收部分 × (平时价−谷价/效率) × 天数。",
...
def _daily_charge_kwh(inp: InputModel) -> float:
    ...
    return _daily_charge_kwh_from_counts(
...
def _daily_charge_kwh_from_counts(
    *,
    n_pile_60: int,
    n_pile_120: int,
    n_ultra_480: int,
    ...
    """按桩数量与单枪日均电量计算全站日总充电量（kWh/日）。"""
    guns = _charger_gun_counts(
        n_pile_60=n_pile_60,
    ...
    return (
        guns["guns_60"] * e_std
        + guns["guns_120"] * e_std
        + guns["guns_480"] * e_ultra
    )


def _charger_gun_counts(*, n_pile_60: int, n_pile_120: int, n_ultra_480: int) -> dict[str, int]:
    """按桩数量换算各类型充电枪/终端数量。"""
    n60 = max(0, int(n_pile_60))
    n120 = max(0, int(n_pile_120))
    n_ultra = max(0, int(n_ultra_480))
    return {
        "guns_60": n60,
        "guns_120": n120 * PILE_120_GUNS_PER_UNIT,
        "guns_480": n_ultra * ULTRA_CHARGE_TERMINALS_PER_RACK,
    }


def _mode1_formula_lines(
    *,
    n60: int,
    n120: int,
    n480: int,
    e_day: float,
    e_ultra_day: float,
    charger_load_day: float,
    peak_share: float,
    peak_capacity_day: float,
    storage_discharge_day: float,
    consumed_peak_day: float,
    remaining_uniform_day: float,
    other_load_kwh_day: float,
    excess_day: float,
    consumed_flat_day: float,
    unconsumed_day: float,
    storage_utilization_ratio: float,
    storage_sets: int,
    storage_kwh_per_set: float,
    cycles: float,
) -> list[str]:
    """模式一展示用：逐行计算说明。"""
    guns = _charger_gun_counts(n_pile_60=n60, n_pile_120=n120, n_ultra_480=n480)
    return [
        f"- 枪数量：60kW **{guns['guns_60']}** 枪，120kW **{guns['guns_120']}** 枪，480kW **{guns['guns_480']}** 终端",
        f"- 日总充电量 = 60枪数×单枪日均 + 120枪数×单枪日均 + 480终端数×单终端日均"
        f" = {guns['guns_60']}×{e_day:.0f} + {guns['guns_120']}×{e_day:.0f} + {guns['guns_480']}×{e_ultra_day:.0f}"
        f" = **{charger_load_day:.0f}** kWh/日",
        f"- 峰时充电量 = 日总充电量 × 峰时充电占比 = {charger_load_day:.0f} × {peak_share:.2f}"
        f" = **{peak_capacity_day:.0f}** kWh/日",
        f"- 储能日放电量 = 储能装机 × 日循环 = {storage_sets * storage_kwh_per_set:.0f} × {cycles:.2f}"
        f" = **{storage_discharge_day:.0f}** kWh/日",
        f"- 峰时消纳 = min(储能日放电量, 峰时充电量)"
        f" = min({storage_discharge_day:.0f}, {peak_capacity_day:.0f})"
        f" = **{consumed_peak_day:.0f}** kWh/日",
        f"- 平时可消纳负荷 = (日总充电量 - 峰时充电量) + 其他均匀负荷"
        f" = ({charger_load_day:.0f} - {peak_capacity_day:.0f}) + {other_load_kwh_day:.0f}"
        f" = **{remaining_uniform_day:.0f}** kWh/日",
        f"- 平时消纳 = min(储能剩余电量, 平时可消纳负荷)"
        f" = min({excess_day:.0f}, {remaining_uniform_day:.0f})"
        f" = **{consumed_flat_day:.0f}** kWh/日",
        f"- 未消纳 = 储能剩余电量 - 平时消纳 = {excess_day:.0f} - {consumed_flat_day:.0f}"
        f" = **{unconsumed_day:.0f}** kWh/日",
        f"- 储能利用率 = (峰时消纳 + 平时消纳) / 储能日放电量"
        f" = ({consumed_peak_day:.0f} + {consumed_flat_day:.0f}) / {storage_discharge_day:.0f}"
        f" = **{storage_utilization_ratio:.2%}**",
    ]
...
def _mode1_consumption_metrics(
    inp: InputModel,
    *,
    storage_discharge_day: float,
    charger_kwh_day: float,
) -> dict[str, float]:
    """模式一日度消纳测算：峰时消纳、平时消纳、未消纳与储能利用率。"""
    peak_share = float(
        np.clip(
            getattr(inp, "peak_charge_share_mode1", MODE1_PEAK_CHARGE_SHARE_DEFAULT),
            0.0,
            1.0,
        )
    )
    other_uniform = max(
        0.0,
        float(getattr(inp, "other_load_kwh_day", 0.0))
        if str(getattr(inp, "has_other_load", "否")) == "是"
        else 0.0,
    )
    charge_day = max(0.0, float(charger_kwh_day))
    discharge_day = max(0.0, float(storage_discharge_day))
    peak_capacity_day = charge_day * peak_share
    consumed_peak_day = min(discharge_day, peak_capacity_day)
    excess_day = max(0.0, discharge_day - consumed_peak_day)
    remaining_uniform_day = max(0.0, charge_day - peak_capacity_day) + other_uniform
    consumed_flat_day = min(excess_day, remaining_uniform_day)
    unconsumed_day = max(0.0, excess_day - consumed_flat_day)
    utilization_ratio = (
        0.0 if discharge_day <= 1e-6 else min(1.0, (consumed_peak_day + consumed_flat_day) / discharge_day)
    )
    return {
        "peak_share": peak_share,
        "peak_capacity_day": peak_capacity_day,
        "consumed_peak_day": consumed_peak_day,
        "consumed_flat_day": consumed_flat_day,
        "unconsumed_day": unconsumed_day,
...
def _annual_revenue_by_usage(
    ...
    *,
    ...
    if inp.usage == USAGE_PEAK_VALLEY_ONLY:
        m1 = _mode1_consumption_metrics(
            inp,
            storage_discharge_day=max(0.0, e_max_day),
            charger_kwh_day=max(0.0, kwh_charge_day),
        )
        eta_cl = max(1e-6, eta)
        peak_margin = peak - valley / eta_cl
        flat_margin = inp.flat_price - valley / eta_cl
        rev["储能峰谷套利(峰时消纳)"] = float(m1["consumed_peak_day"]) * peak_margin * days
        if float(m1["consumed_flat_day"]) > 1e-6:
            rev["储能峰谷套利(平时消纳)"] = float(m1["consumed_flat_day"]) * flat_margin * days
        if float(m1["unconsumed_day"]) > 1e-6:
            rev["储能未消纳电量(提示)"] = 0.0
...
def _storage_revenue_line_keys(usage: str) -> set[str]:
    ...
    if usage == USAGE_PEAK_VALLEY_ONLY:
        return {"储能峰谷套利(峰时消纳)", "储能峰谷套利(平时消纳)"}
...
def _daily_charge_kwh_breakdown(
    ...
    e_ultra = max(0.0, float(ultra_energy_per_gun_day))
    p60 = n60 * e_std
    p120 = n120 * PILE_120_GUNS_PER_UNIT * e_std
    pultra = nu * ULTRA_CHARGE_TERMINALS_PER_RACK * e_ultra
    total = p60 + p120 + pultra
    detail = (
        f"60kW {n60} 台×{e_std:.0f} + 120kW {n120} 台×{PILE_120_GUNS_PER_UNIT}×{e_std:.0f}"
        f" + 480kW {nu} 套×{ULTRA_CHARGE_TERMINALS_PER_RACK}×{e_ultra:.0f}（单终端日均）"
        f" = <strong>{total:.0f}</strong> kWh/天"
    )
    return total, detail


def _html_revenue_calc_box(lines: list[str]) -> None:
    """收入计算过程说明框（行内统一 HTML，**粗体** 会转为 strong）。"""
    if not lines:
        return
    inner = "".join(f"<p>{_md_emphasis_to_html(line)}</p>" for line in lines)
...
def _storage_revenue_calc_lines(
    ...
    *,
    ...
    peak_charge_share_mode1: float = MODE1_PEAK_CHARGE_SHARE_DEFAULT,
    has_other_load: str = "否",
    other_load_kwh_day: float = 0.0,
) -> list[str]:
    """储能相关年化收入计算过程。"""
    spread = max(0.0, peak - valley)
    lines: list[str] = []
    ...
    if usage_s == USAGE_PEAK_VALLEY_ONLY:
        helper_inp = SimpleNamespace(
            peak_charge_share_mode1=peak_charge_share_mode1,
            has_other_load=has_other_load,
            other_load_kwh_day=other_load_kwh_day,
        )
        m1 = _mode1_consumption_metrics(
            helper_inp,
            storage_discharge_day=storage_kwh * cyc,
            charger_kwh_day=kwh_day,
        )
        peak_margin = peak - valley / max(1e-6, eta)
        ann_peak = float(rev_bd.get("储能峰谷套利(峰时消纳)", float(m1["consumed_peak_day"]) * peak_margin * days))
        ann_flat = float(rev_bd.get("储能峰谷套利(平时消纳)", 0.0))
        lines.extend(
            [
                f"<strong>峰时充电量</strong> = 日总充电量 × 峰时占比 = {kwh_day:.0f} × {peak_charge_share_mode1:.0%}"
                f" = <code>{m1['peak_capacity_day']:.0f}</code> kWh/日",
                f"<strong>储能峰时消纳</strong> = min(储能日放电, 峰时充电量) = "
                f"min({storage_kwh * cyc:.0f}, {m1['peak_capacity_day']:.0f}) = <code>{m1['consumed_peak_day']:.0f}</code> kWh/日",
                f"<strong>峰时套利年化</strong> = 峰时消纳 × (峰价−谷价/效率) × 天数"
                f" = <code>{ann_peak:,.0f}</code> 元/年",
                f"<strong>平时消纳套利年化</strong> = <code>{ann_flat:,.0f}</code> 元/年",
                f"<strong>储能利用率</strong> = {(float(m1['utilization_ratio'])):.2%}",
            ]
        )
        return lines
...
def render_storage_operations_preview(
    *,
    ...
    spread = max(0.0, peak - valley)
    fee = max(0.0, float(service_fee_per_kwh))
    kwh_day = _daily_charge_kwh_from_counts(
        n_pile_60=n_pile_60,
        n_pile_120=n_pile_120,
        n_ultra_480=n_ultra_480,
        energy_per_gun_day=energy_per_gun_day,
    ...
    stub = SimpleNamespace(
        ...
        has_other_load=str(has_other_load),
        other_load_kwh_day=float(other_load_kwh_day),
        peak_charge_share_mode1=float(peak_charge_share_mode1),
    )
...
def compute_projection(inp: InputModel) -> dict[str, Any]:
    ...
    mode1_util = _mode1_consumption_metrics(
        inp,
        storage_discharge_day=max(0.0, storage_kwh_total * max(0.0, float(inp.storage_cycles_per_day)) * battery_soh_factor(1)),
        charger_kwh_day=kwh_charge_day,
    )
    ...
    return {
        ...
        "meta": {
            ...
            "模式一·峰时充电占比": float(np.clip(inp.peak_charge_share_mode1, 0.0, 1.0)),
            "储能利用率": float(mode1_util["utilization_ratio"])
            if inp.usage == USAGE_PEAK_VALLEY_ONLY and str(inp.storage_installed) != STORAGE_INSTALL_NO
            else None,
...
def render_input_form(defs: dict[str, Any]) -> tuple[bool, InputModel | None, list[str]]:
    ...
    _html_step_title("第一部分：场站资源配置方案")
    _html_flow_zone_open("1. 充电桩配置")
    st.caption("1.1 桩型与功率规划：a) 按剩余容量规划；b) 分体式液冷480kW；c) 60/120/480 推荐搭配。")
    ...
    has_other_load = str(d.get("has_other_load", "否"))
    other_load_kwh_day_input = float(d.get("other_load_kwh_day", 0.0))
    peak_charge_share_mode1 = float(d.get("peak_charge_share_mode1", MODE1_PEAK_CHARGE_SHARE_DEFAULT))
    storage_utilization_ratio_mode1 = 0.0
    ...
    _html_flow_zone_open("1.2 数量与单价")
    ...
    _html_flow_zone_close()
    _html_flow_zone_open("1.3 纯桩运营参数")
    ...
    with op1:
        d_op = float(
            st.number_input(
                ...
                step=1.0,
                key="pre_op_charger_operating_days",
            )
        )
    with op2:
        fee = float(
            st.number_input(
                ...
                step=0.01,
                key="pre_op_charger_service_fee",
            )
        )
    ...
    with op3:
        e_day = float(
            st.number_input(
                ...
                step=10.0,
                key="pre_op_charger_energy_per_gun",
            )
        )
    with op4:
        e_ultra_day = float(
            st.number_input(
                ...
                step=10.0,
                key="pre_op_charger_ultra_energy",
            )
        )
    ...
    _html_step_divider()
    _html_flow_zone_open("2. 容量校核和增容缺口")
    req_step3_kva = required_transformer_kva(
        _pile_config_total_kw(n60, n120, n480), plan_kx, plan_kt, plan_cos, plan_eta
    )
    gap_step3 = charger_capacity_gap_kva(req_step3_kva, k_remain)
    total_kw_step3 = _pile_config_total_kw(n60, n120, n480)
    capacity_ok = gap_step3 <= 1e-3
    st.markdown(
        f"- 规划需用变压器：**{req_step3_kva:.1f} kVA**；剩余可用：**{k_remain:.1f} kVA**\n"
        f"- 容量缺口：**{gap_step3:.1f} kVA**；装机功率：**{total_kw_step3:.0f} kW**"
    )
    _html_substep_title("2.2 容量配置校验")
    sec2_storage_selected = str(charger_plan.get("storage_installed", STORAGE_INSTALL_NO)) == STORAGE_INSTALL_YES if is_mode_b else False
    b_target_kva_sec2 = float(
        st.session_state.get(
            "charger_plan_b_k_target",
            charger_plan.get("k_target", baseline_kva),
        )
    ) if is_mode_b else baseline_kva
    sec2_tf_expansion_planned = b_target_kva_sec2 > baseline_kva + 1e-3 if is_mode_b else False
    sec2_ready = True
    sec2_msg = "容量校验通过。"
    pre_sets = int(charger_plan.get("storage_sets", max(1, int(d.get("storage_sets", 1))))) if is_mode_b else max(1, int(d.get("storage_sets", 1)))
    pre_peak_red_kw = _effective_peak_reduction_kw(
        float(d.get("peak_reduction_kw", 0.0)), pre_sets
    )
    pre_storage_cost_wan = max(0.0, pre_sets) * float(
        charger_plan.get("storage_kwh_per_set", d.get("storage_kwh_per_set", STORAGE_KWH_PER_SET_DEFAULT))
        if is_mode_b
        else d.get("storage_kwh_per_set", STORAGE_KWH_PER_SET_DEFAULT)
    ) * float(
        charger_plan.get("price_storage_per_kwh", d.get("price_storage_per_kwh", 0.65))
        if is_mode_b
        else d.get("price_storage_per_kwh", 0.65)
    ) * 1000.0 / 1e4
    need_target_kva = baseline_kva + gap_step3
    tf_upgrade_cost_wan = max(0.0, need_target_kva - baseline_kva) * float(pu) / 1e4
    sec2_storage_only_scheme = False

    if not is_mode_b:
        if capacity_ok:
            sec2_msg = "按A配置：未超过变压器剩余容量，跳过第2节，直接进入第3节。"
            st.success(sec2_msg)
        else:
            sec2_ready = False
            sec2_msg = "按A配置：桩配置超过变压器剩余容量。"
            st.warning("High light：当前配置超出剩余容量，请返回上层修改桩配置信息。")
    else:
        if capacity_ok:
            st.success("按B配置：桩配置未超过原变压器容量，跳过本节。")
        else:
            k_target = b_target_kva_sec2
            if sec2_storage_selected and (not sec2_tf_expansion_planned):
                dyn_cov_pre = assess_dynamic_storage_coverage(
                    peak_reduction_kw=pre_peak_red_kw,
                    storage_sets=pre_sets,
                    req_kva=req_step3_kva,
                    kva_remaining=k_remain,
                    power_factor=pf,
                )
                if dyn_cov_pre["covers"]:
                    sec2_storage_only_scheme = True
                    render_storage_only_vs_transformer_advisory(
                        baseline_kva=baseline_kva,
                        need_target_kva=need_target_kva,
                        tf_upgrade_cost_wan=tf_upgrade_cost_wan,
                        storage_cost_wan=pre_storage_cost_wan,
                        storage_covers=True,
                        has_tf_expansion_planned=False,
                    )
                else:
                    sec2_ready = False
                    sec2_msg = "储能动态增容不足，需变压器增容。"
                    st.warning(
                        f"High light：储能暂无法覆盖缺口（缺口 {dyn_cov_pre['gap_kw']:.0f} kW，储能功率 {dyn_cov_pre['storage_kw']:.0f} kW，削峰 {dyn_cov_pre['shave_kw']:.0f} kW），请返回上层修改。"
                    )
            elif (not sec2_storage_selected) and sec2_tf_expansion_planned:
                if b_target_kva_sec2 < need_target_kva - 1e-6:
                    sec2_ready = False
                    st.warning(
                        f"仅增容方案不满足：目标容量 {b_target_kva_sec2:.0f} kVA < 所需 {need_target_kva:.0f} kVA，请返回上层修改。"
                    )
                else:
                    st.success(
                        f"仅增容方案满足：目标容量 {b_target_kva_sec2:.0f} kVA ≥ 所需 {need_target_kva:.0f} kVA。"
                    )
            elif sec2_storage_selected and sec2_tf_expansion_planned:
                dyn_cov_pre = assess_dynamic_storage_coverage(
                    peak_reduction_kw=pre_peak_red_kw,
                    storage_sets=pre_sets,
                    req_kva=req_step3_kva,
                ...
                if dyn_cov_pre["covers"]:
                    render_storage_only_vs_transformer_advisory(
                        ...
                        storage_cost_wan=pre_storage_cost_wan,
                        storage_covers=True,
                        has_tf_expansion_planned=True,
                    )
                else:
                    st.warning("储能单独无法覆盖缺口。")
                    st.caption(
                        f"仅变压器增容估算：**{tf_upgrade_cost_wan:.2f} 万元**；"
                        f"当前储能估算：**{pre_storage_cost_wan:.2f} 万元**。"
                    )
            else:
                sec2_ready = False
                sec2_msg = "按B配置：超容但未选增容/储能，无法通过校验。"
                st.warning("请返回上层在B模式中至少选择一种能力补偿方式。")
    st.session_state["sec2_storage_only_scheme"] = bool(sec2_storage_only_scheme)
    _html_flow_zone_close()

    _html_step_divider()
    _html_flow_zone_open("3. 储能配置")
    r_col_l, r_col_r = st.columns([4, 1])
    with r_col_r:
        if st.button("刷新第3部分联动", key="sec3_refresh_btn"):
            st.session_state["sec3_refresh_ts"] = pd.Timestamp.now().strftime("%H:%M:%S")
            st.rerun()
    with r_col_l:
        ts3 = st.session_state.get("sec3_refresh_ts")
        if ts3:
            st.caption(f"已按第1-2部分配置刷新：{ts3}")
    _html_substep_title("3.1 储能安装联动判断")
    if not sec2_ready:
        st.error("进入储能配置前，请先在第2节完成容量最优配置确认。")
    auto_storage_reason = ""
    if capacity_ok:
        auto_storage_reason = "容量满足：可手动选择是否安装工商业储能。"
        storage_install = st.radio(
            "是否安装工商业储能",
            options=STORAGE_INSTALL_OPTIONS,
            index=_select_index(
        ...
        if sec2_storage_selected:
            storage_install = STORAGE_INSTALL_YES
            install_storage = True
            auto_storage_reason = "容量不足且选择“配置储能削峰（动态增容）”：已自动设为安装工商业储能。"
        else:
            storage_install = STORAGE_INSTALL_NO
            install_storage = False
            auto_storage_reason = "容量不足且未选择储能削峰：当前结论为不配置工商业储能（优先降额或变压器增容）。"
    st.caption(auto_storage_reason)
    if is_mode_b and install_storage:
        sets = int(charger_plan.get("storage_sets", 0))
        e_set = float(charger_plan.get("storage_kwh_per_set", STORAGE_KWH_PER_SET_DEFAULT))
        st.caption(
            f"自定义模式已配置储能规模：**{sets}** 套 × {e_set:.0f} kWh × {STORAGE_KW_PER_SET_DEFAULT:.0f} kW/套。"
        )
    ...
    if not install_storage:
        ...
        if (not capacity_ok) and sec2_storage_selected:
        ...
        _html_substep_title("3.2 储能运营模式")
        b_target_mode = float(charger_plan.get("k_target", baseline_kva)) if is_mode_b else float(k_target)
        storage_only_scheme = bool(st.session_state.get("sec2_storage_only_scheme", False))
        if is_mode_b and sec2_storage_selected and b_target_mode <= baseline_kva + 1e-3:
            dyn_cov_mode = assess_dynamic_storage_coverage(
                peak_reduction_kw=_effective_peak_reduction_kw(
                    float(d.get("peak_reduction_kw", 0.0)), sets
                ),
                storage_sets=sets,
                req_kva=req_m3_advisory,
                kva_remaining=k_remain,
                power_factor=pf,
            )
            storage_only_scheme = bool(dyn_cov_mode["covers"])
        usage_default = str(
            st.session_state.get("storage_usage_select", str(d.get("usage", USAGE_PEAK_VALLEY_ONLY)))
        )
        decision_mode_default = usage_to_decision_mode(usage_default)
        if storage_only_scheme:
            st.info(
                f"第 2.2 节为**仅储能补偿**（目标变压器未增容），运营模式固定为 **{USAGE_DECISION_MODE2}**。"
            )
            decision_mode = USAGE_DECISION_MODE2
            st.selectbox(
                "运营模式",
                options=USAGE_DECISION_OPTIONS,
                index=_select_index(USAGE_DECISION_OPTIONS, USAGE_DECISION_MODE2, USAGE_DECISION_MODE2),
                help="仅储能覆盖缺口时须用动态增容优先，不可选峰谷套利优先。",
                key="storage_usage_decision_select",
                disabled=True,
            )
        else:
            decision_mode = st.selectbox(
                ...
                options=USAGE_DECISION_OPTIONS,
                index=_select_index(
                    USAGE_DECISION_OPTIONS, decision_mode_default, USAGE_DECISION_MODE1
                ),
                help="模式一=峰谷套利优先；模式二=动态增容优先（削峰为主）。",
                key="storage_usage_decision_select",
            )
        usage = decision_mode_to_usage(decision_mode)
        st.session_state["storage_usage_select"] = usage
        peak_charge_share_mode1 = float(
            st.number_input(
                "峰时充电占比(0-1)",
                min_value=0.0,
                max_value=1.0,
                value=float(np.clip(peak_charge_share_mode1, 0.0, 1.0)),
                step=0.01,
                key="mode1_peak_charge_share",
            )
        )
        st.caption("建议：白天繁忙型 70%~80%；全天平均型 50%~60%；夜间集中型 30%~40%。")
        has_other_load = st.selectbox(
            "是否有其他负荷接入",
            options=HP_OPTIONS,
            index=_select_index(HP_OPTIONS, has_other_load, "否"),
            key="storage_has_other_load",
        )
        other_load_kwh_day = 0.0
        if has_other_load == "是":
            other_load_kwh_day_input = float(
                st.number_input(
                    "其他负荷日耗电量(kWh)",
                    min_value=0.0,
                    value=float(other_load_kwh_day_input),
                    step=50.0,
                    key="storage_other_load_kwh_day",
                )
            )
            other_load_kwh_day = other_load_kwh_day_input
        charger_load_day = _daily_charge_kwh_from_counts(
            n_pile_60=n60,
            n_pile_120=n120,
            n_ultra_480=n480,
            energy_per_gun_day=e_day,
        ...
        storage_discharge_day = max(0.0, sets * e_set * cyc)
        mode1_metrics = _mode1_consumption_metrics(
            SimpleNamespace(
                has_other_load=has_other_load,
                other_load_kwh_day=other_load_kwh_day,
                peak_charge_share_mode1=peak_charge_share_mode1,
            ),
            storage_discharge_day=storage_discharge_day,
            charger_kwh_day=charger_load_day,
        )
        peak_capacity_day = float(mode1_metrics["peak_capacity_day"])
        remaining_uniform_day = max(
            0.0,
            (charger_load_day - peak_capacity_day) + other_load_kwh_day,
        )
        consumed_peak_day = float(mode1_metrics["consumed_peak_day"])
        consumed_flat_day = float(mode1_metrics["consumed_flat_day"])
        unconsumed_day = float(mode1_metrics["unconsumed_day"])
        excess_day = max(0.0, storage_discharge_day - consumed_peak_day)
        storage_utilization_ratio_mode1 = float(mode1_metrics["utilization_ratio"])
        if usage == USAGE_PEAK_VALLEY_ONLY:
            st.caption(
                f"模式一：峰时消纳 **{consumed_peak_day:.0f}** kWh/日，平时消纳 **{consumed_flat_day:.0f}** kWh/日，"
                f"未消纳 **{unconsumed_day:.0f}** kWh/日；储能利用率 **{storage_utilization_ratio_mode1:.2%}**。"
            )
            with st.expander("模式一数值计算过程", expanded=False):
                c_l, c_r = st.columns([4, 1])
                with c_r:
                    if st.button("刷新联动", key="mode1_calc_refresh_btn"):
                        st.session_state["mode1_calc_refresh_ts"] = pd.Timestamp.now().strftime("%H:%M:%S")
                        st.rerun()
                with c_l:
                    ts = st.session_state.get("mode1_calc_refresh_ts")
                    if ts:
                        st.caption(f"已按当前桩配置与其他负荷刷新：{ts}")
                for line in _mode1_formula_lines(
                    n60=n60,
                    n120=n120,
                    n480=n480,
                    e_day=e_day,
                    e_ultra_day=e_ultra_day,
                    charger_load_day=charger_load_day,
                    peak_share=peak_charge_share_mode1,
                    peak_capacity_day=peak_capacity_day,
                    storage_discharge_day=storage_discharge_day,
                    consumed_peak_day=consumed_peak_day,
                    remaining_uniform_day=remaining_uniform_day,
                    other_load_kwh_day=other_load_kwh_day,
                    excess_day=excess_day,
                    consumed_flat_day=consumed_flat_day,
                    unconsumed_day=unconsumed_day,
                    storage_utilization_ratio=storage_utilization_ratio_mode1,
                    storage_sets=sets,
                    storage_kwh_per_set=e_set,
                    cycles=cyc,
                ):
            ...
            st.caption(
                f"模式二：优先削峰；当前总负荷 **{(charger_load_day + other_load_kwh_day):.0f} kWh/日**，"
                f"储能日放电能力 **{storage_discharge_day:.0f} kWh/日**。"
            )
        is_dynamic_usage = usage == USAGE_DYNAMIC_CAPACITY

        if is_dynamic_usage:
            _html_substep_title("动态增容参数")
            peak_red_default = float(d.get("peak_reduction_kw", 0.0))
            if storage_only_scheme:
                gap_kw_hint = gap_m3_advisory * max(0.0, float(pf))
                peak_red_default = max(
                    peak_red_default,
                    _effective_peak_reduction_kw(0.0, sets),
                    gap_kw_hint,
                )
            ...
            with dyn1:
                peak_red_kw = float(
                    st.number_input(
                        "动态增容值(kW)",
                        min_value=0.0,
                        value=peak_red_default,
                        step=10.0,
                        help="储能削峰可减少的峰值需量/负荷（kW）；仅储能方案建议≥容量缺口 kW。",
                    )
                )
            with dyn2:
    ...
    _html_step_divider()
    _html_flow_zone_open("4. 最终总结输出：场站资源配置方案")
    transformer_plan_txt = "不增容" if max(0.0, k_target - baseline_kva) <= 1e-6 else f"增容到 {k_target:.0f} kVA"
    storage_mode_txt = usage_to_decision_mode(usage) if install_storage else "不配置储能"
    mode1_ratio_txt = (
        f"；储能利用率：**{storage_utilization_ratio_mode1:.2%}**"
        if install_storage and usage == USAGE_PEAK_VALLEY_ONLY
        else ""
    )
    st.markdown(
        f"- 桩配置：60kW×**{n60}**、120kW×**{n120}**、480kW×**{n480}**\n"
        f"- 变压器方案：**{transformer_plan_txt}**（基准 {baseline_kva:.0f} kVA）\n"
        f"- 储能配置：**{sets}** 套 × {e_set:.0f} kWh；运营模式：**{storage_mode_txt}**{mode1_ratio_txt}"
    )
    _html_flow_zone_close()

    _html_step_divider()
    _html_step_title("第二部分：施工成本")
    st.caption(
    ...
    _html_step_divider()
    _html_step_title("第三部分：运营与测算")
    st.caption(
        "顺序：**储能运营 → 政府补贴 → 全站假设**；充电桩运营参数已前置到第二步。"
    )

    _html_flow_zone_open("5.1 运营参数")
    st.caption(
        f"充电桩运营参数已在第二步确认：运营天数 **{d_op:.0f}**、服务费 **{fee:.2f} 元/kWh**、"
        f"常规单枪 **{e_day:.0f} kWh/日**、超充单终端 **{e_ultra_day:.0f} kWh/日**。"
    )
    ...
    if (
        plan_mode == CHARGER_CONFIG_MODE_B
        and install_storage
        and str(charger_plan.get("storage_installed", STORAGE_INSTALL_NO)) == STORAGE_INSTALL_YES
        and float(charger_plan.get("k_target", baseline_kva)) <= baseline_kva + 1e-3
        and assess_dynamic_storage_coverage(
            peak_reduction_kw=_effective_peak_reduction_kw(peak_red_kw, sets),
            storage_sets=sets,
            req_kva=required_transformer_kva(
                _pile_config_total_kw(n60, n120, n480), plan_kx, plan_kt, plan_cos, plan_eta
            ),
            kva_remaining=k_remain,
            power_factor=pf,
        ...
        errs.append(
            f"仅储能补偿（目标变压器未增容且储能可覆盖缺口）须选择「{USAGE_DECISION_MODE2}」。"
        )
    ...
    if not sec2_ready:
        errs.append("第2节容量配置校验未通过，请先完成最优配置确认。")
...
def build_scheme_product_config(inp: InputModel) -> dict[str, Any]:
    """
    方案产品配置快照：设备台/套数与车位占用（供保存前预览与方案对比）。
    作者: zhi.qu
    """
    n60 = max(0, int(inp.n_pile_60))
    n120 = max(0, int(inp.n_pile_120))
    n480 = max(0, int(inp.n_ultra_480))
    spots_total = max(0, int(inp.parking_spots))
    spots_used = charger_parking_spots_used(n60, n120, n480)
    install_storage = str(inp.storage_installed) == STORAGE_INSTALL_YES
    sets = max(0, int(inp.storage_sets)) if install_storage else 0
    kwh_per_set = float(inp.storage_kwh_per_set or STORAGE_KWH_PER_SET_DEFAULT)

    products: list[dict[str, Any]] = []
    if n60 > 0:
        products.append({"label": "60kW 桩", "count": n60, "unit": "台"})
    if n120 > 0:
        products.append({"label": "120kW 桩", "count": n120, "unit": "台"})
    if n480 > 0:
        products.append({"label": "480kW 液冷", "count": n480, "unit": "套"})
    if sets > 0:
        products.append(
            {
                "label": "工商业储能",
                "count": sets,
                "unit": "套",
                "extra": f"{sets * kwh_per_set:.0f} kWh",
            }
        )
    ...
    layout_slots = max(spots_total, spots_used)
    storage_kwh_total = sets * kwh_per_set if sets > 0 else 0.0
    return {
        "parking_spots_total": spots_total,
        "parking_spots_used": spots_used,
        "parking_spots_free": max(0, spots_total - spots_used),
        "storage_installed": install_storage,
        "storage_kwh_per_set": kwh_per_set,
        "storage_kwh_total": storage_kwh_total,
        "usage": str(inp.usage),
        "products": products,
        "counts": counts,
        "layout_cells": build_scheme_layout_cells(counts, layout_slots),
    }


_SCHEME_LAYOUT_COLS = 4
_PILE_LAYOUT_META: dict[int, tuple[str, str]] = {
    60: ("60kW", "#0f766e"),
    120: ("120kW", "#0369a1"),
    480: ("480kW 液冷", "#7c3aed"),
}
...
def build_scheme_layout_cells(
    counts: dict[str, Any], spots_total: int
) -> list[dict[str, Any]]:
    """
    按规划顺序生成车位格：480 套(4 位) → 120 桩(2 位) → 60 桩(1 位) → 空余位。
    作者: zhi.qu
    """
    cells: list[dict[str, Any]] = []
    gid = 0

    def _append_pile_group(kw: int, group_size: int, n_groups: int) -> None:
        nonlocal gid, cells
        label, color = _PILE_LAYOUT_META[kw]
        for _ in range(max(0, int(n_groups))):
                ...
                cells.append(
                    {
                        ...
                        "pile_color": color,
                        "pile_group": gid,
                        "pile_group_start": idx == 0,
                        "pile_group_size": group_size,
                    }
                )
            gid += 1

    _append_pile_group(480, ULTRA_PARKING_SPOTS_PER_SET, int(counts.get("ultra_480", 0)))
    _append_pile_group(120, PILE_120_SPOTS_PER_UNIT, int(counts.get("pile_120", 0)))
    _append_pile_group(60, 1, int(counts.get("pile_60", 0)))

    target = max(0, int(spots_total))
    while len(cells) < target:
        cells.append({"state": "empty", "slot": len(cells) + 1})
    return cells


def _scheme_layout_row_span(row: list[dict[str, Any]], col: int) -> int:
    """当前行内从 col 起同组车位连续占列数（最多至行末）。"""
    if col >= len(row):
        return 1
    c0 = row[col]
    if c0.get("state") != "pile":
        return 1
    gid = c0.get("pile_group")
    span = 1
    while col + span < len(row) and row[col + span].get("pile_group") == gid:
        span += 1
    return span


def _scheme_layout_site_plan_html(cfg: dict[str, Any]) -> str:
    """场站平面示意：每行 4 车位，桩在车位前方并标注功率。"""
    counts = cfg.get("counts") or {}
    cells = cfg.get("layout_cells")
    if not cells:
        slots = max(
            int(cfg.get("parking_spots_total", 0)),
            int(cfg.get("parking_spots_used", 0)),
        )
        cells = build_scheme_layout_cells(counts, slots)

    if not cells:
        return '<p class="ui-tone-neutral">暂无车位布局（请填写场站车位与桩数）。</p>'

    rows: list[str] = []
    for row_start in range(0, len(cells), _SCHEME_LAYOUT_COLS):
        row = list(cells[row_start : row_start + _SCHEME_LAYOUT_COLS])
        while len(row) < _SCHEME_LAYOUT_COLS:
            row.append({"state": "pad"})

        pile_parts: list[str] = []
        spot_parts: list[str] = []
        col = 0
        while col < _SCHEME_LAYOUT_COLS:
            c = row[col]
            if c.get("state") == "pad":
                pile_parts.append('<div class="site-pile-spacer"></div>')
                spot_parts.append('<div class="site-spot site-spot-pad"></div>')
                col += 1
                continue
            if c.get("state") == "empty":
                pile_parts.append('<div class="site-pile-spacer"></div>')
                sn = int(c.get("slot", 0))
                spot_parts.append(
                    f'<div class="site-spot site-spot-empty" title="空余车位">'
                    f'<span class="site-spot-no">{sn}</span><span class="site-spot-tag">空</span></div>'
                )
                col += 1
                continue
            if c.get("pile_group_start"):
                span = _scheme_layout_row_span(row, col)
                label = html.escape(str(c.get("pile_label", "")))
                color = html.escape(str(c.get("pile_color", "#0f766e")))
                pile_parts.append(
                    f'<div class="site-pile" style="grid-column:span {span};'
                    f'--pile-color:{color}" title="{label}">'
                    f'<span class="site-pile-gun"></span>'
                    f'<span class="site-pile-label">{label}</span></div>'
                )
                gid = c.get("pile_group")
                for j in range(span):
                    cc = row[col + j]
                    sn = int(cc.get("slot", 0))
                    spot_parts.append(
                        f'<div class="site-spot site-spot-on" style="--pile-color:{color}">'
                        f'<span class="site-spot-no">{sn}</span></div>'
                    )
                col += span
            else:
                pile_parts.append('<div class="site-pile-spacer"></div>')
                cc = row[col]
                color = html.escape(str(cc.get("pile_color", "#0f766e")))
                sn = int(cc.get("slot", 0))
                spot_parts.append(
                    f'<div class="site-spot site-spot-on" style="--pile-color:{color}">'
                    f'<span class="site-spot-no">{sn}</span></div>'
        ...
        row_no = row_start // _SCHEME_LAYOUT_COLS + 1
        rows.append(
            f'<div class="site-plan-row">'
            f'<div class="site-plan-row-label">第 {row_no} 排</div>'
            f'<div class="site-pile-row">{"".join(pile_parts)}</div>'
            f'<div class="site-spot-row">{"".join(spot_parts)}</div>'
            f"</div>"
        )

    storage_sets = int(counts.get("storage_sets", 0))
    storage_html = ""
    if storage_sets > 0:
        usage = html.escape(str(cfg.get("usage", "")).strip())
        storage_html = (
            f'<div class="site-storage">'
            f'<div class="site-storage-box"><span class="site-storage-icon">🔋</span>'
            f'<span>工商业储能 × {storage_sets} 套</span></div>'
            + (
                f'<span class="site-storage-usage">{usage}</span>'
                if usage
                else ""
            )
            + "</div>"
    ...
    legend = (
        ...
        '<span><i class="lg lg-60"></i>60kW</span>'
        '<span><i class="lg lg-120"></i>120kW</span>'
        '<span><i class="lg lg-480"></i>480kW 液冷</span>'
        '<span><i class="lg lg-empty"></i>空余</span>'
        "</div>"
    )
...
.site-plan-row {{ margin-bottom: 14px; }}
.site-plan-row:last-child {{ margin-bottom: 6px; }}
.site-plan-row-label {{
  font-size: 0.72rem;
  color: #94a3b8;
  margin-bottom: 4px;
}}
.site-pile-row, .site-spot-row {{
  display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: 10px;
}}
.site-pile-row {{ min-height: 44px; align-items: end; margin-bottom: 6px; }}
.site-pile-spacer {{ min-height: 1px; }}
.site-pile {{
  display: flex;
  ...
  background: linear-gradient(180deg, color-mix(in srgb, var(--pile-color) 88%, #fff) 0%, var(--pile-color) 100%);
  color: #fff;
  font-size: 0.78rem;
  font-weight: 700;
  box-shadow: 0 3px 6px rgba(15,23,42,0.18);
  border: 1px solid color-mix(in srgb, var(--pile-color) 70%, #0f172a);
}}
.site-pile-gun {{
  width: 14px;
  height: 14px;
...
.site-pile-label {{ line-height: 1.15; text-align: center; }}
.site-spot {{
  position: relative;
  height: 52px;
  border-radius: 6px;
...
.site-spot-pad {{ visibility: hidden; border: none; box-shadow: none; }}
.site-spot-empty {{
  background: repeating-linear-gradient(
    -45deg, #f1f5f9, #f1f5f9 6px, #e8edf3 6px, #e8edf3 12px
  );
  border-style: dashed;
  border-color: #94a3b8;
}}
.site-spot-on {{
  border-color: var(--pile-color);
  background: linear-gradient(180deg, #fff 0%, color-mix(in srgb, var(--pile-color) 12%, #fff) 100%);
}}
.site-spot-no {{
  font-size: 0.8rem;
  font-weight: 600;
...
.site-storage-usage {{ font-size: 0.78rem; color: #64748b; }}
.site-plan-legend {{
  display: flex;
  flex-wrap: wrap;
  gap: 12px 18px;
...
.site-plan-legend span {{ display: inline-flex; align-items: center; gap: 5px; }}
.site-plan-legend .lg {{
...
.lg-60 {{ background: #0f766e; }}
.lg-120 {{ background: #0369a1; }}
.lg-480 {{ background: #7c3aed; }}
.lg-empty {{
  background: #f1f5f9;
...
<div class="site-plan-wrap">
  <div class="site-plan-title">场站平面示意 · 桩在车位前（每行 4 位）</div>
  {"".join(rows)}
  {storage_html}
  {legend}
  <div class="site-plan-hint">自上而下：充电桩 → 车位；同色为同一台（套）桩服务的车位。</div>
</div>
"""


def render_scheme_product_config_panel(cfg: dict[str, Any]) -> None:
    """展示方案产品配置：数字摘要 + 场站平面示意图。"""
    counts = cfg.get("counts") or {}
    spots_total = int(cfg.get("parking_spots_total", 0))
    spots_used = int(cfg.get("parking_spots_used", 0))
    n480 = int(counts.get("ultra_480", 0))
    storage_sets = int(counts.get("storage_sets", 0))
    kwh_per_set = float(
        cfg.get("storage_kwh_per_set") or STORAGE_KWH_PER_SET_DEFAULT
    )
    kwh_total = float(cfg.get("storage_kwh_total", 0.0))
    if storage_sets > 0 and kwh_total < 1e-6:
        kwh_total = storage_sets * kwh_per_set

    r1a, r1b, r1c, r1d = st.columns(4)
    r1a.metric("场站车位", spots_total)
    r1b.metric("桩占车位", f"{spots_used}/{spots_total}")
    r1c.metric("60kW 桩", int(counts.get("pile_60", 0)))
    r1d.metric("120kW 桩", int(counts.get("pile_120", 0)))

    r2a, r2b = st.columns(2)
    r2a.metric("480kW 液冷", f"{n480} 套" if n480 > 0 else "—")
    if storage_sets > 0:
        r2b.metric(
            f"{kwh_total:.0f}kWh 工商业储能",
            f"{storage_sets} 套",
            help=f"单套 {kwh_per_set:.0f} kWh × {storage_sets} 套",
        )
    else:
        r2b.metric("工商业储能", "—")
    if spots_used > spots_total > 0:
        st.caption("⚠ 桩占用车位超过场站车位，落地需扩充车位（测算仍可进行）。")
    st.markdown(_scheme_layout_site_plan_html(cfg), unsafe_allow_html=True)
...
# 右侧投资分项：明细行数较多时默认收起的工程块
_CAPEX_DETAIL_COLLAPSE_SECTIONS = frozenset({"充电桩工程", "储能工程"})


def _render_capex_section_detail_lines(detail_items: list[tuple[str, float]]) -> None:
    """渲染工程块明细列表（万元）。"""
...
def _render_capex_breakdown_grouped(res: dict[str, Any]) -> None:
    ...
    for name in section_order:
        ...
        if not detail_items:
            continue
        if name in _CAPEX_DETAIL_COLLAPSE_SECTIONS:
            with st.expander(
                f"分项明细（{len(detail_items)} 项）",
                expanded=False,
            ):
                _render_capex_section_detail_lines(detail_items)
        else:
            _render_capex_section_detail_lines(detail_items)
        ...
        if storage_rev > 1e-6 or str(inp.storage_installed) != STORAGE_INSTALL_NO:
            ...
            _html_revenue_calc_box(
                _storage_revenue_calc_lines(
                    ...
                    peak_charge_share_mode1=float(inp.peak_charge_share_mode1),
                    has_other_load=str(inp.has_other_load),
                    other_load_kwh_day=float(inp.other_load_kwh_day),
                )
            )
        if usage == USAGE_HYBRID:
...
"""
    ...
    product_cfg = build_scheme_product_config(inp)
    with st.expander("当前方案 · 产品与车位", expanded=True):
        render_scheme_product_config_panel(product_cfg)
        ...
        st.session_state.schemes.append(
            {
                ...
                "storage_utilization": (res.get("meta") or {}).get("储能利用率"),
            }
        )
        st.success("已保存（含产品与车位配置）。")

    if st.session_state.schemes:
        def _scheme_product_row(s: dict[str, Any]) -> dict[str, Any]:
            pc = s.get("product_config") or {}
            cnt = pc.get("counts") or {}
            return {
                "方案": s["name"],
                "车位(占/总)": (
                    f"{int(pc.get('parking_spots_used', 0))}/"
                    f"{int(pc.get('parking_spots_total', 0))}"
                    if pc
                    else "—"
                ),
                "60kW": cnt.get("pile_60", "—"),
                "120kW": cnt.get("pile_120", "—"),
                "480": cnt.get("ultra_480", "—"),
                "储能套": cnt.get("storage_sets", "—"),
                "总投资(万元)": s["capex_wan"],
                "年净收益(万元)": s["net_wan"],
                ...
                "IRR(%)": s["irr_percent"],
                "储能利用率": (
                    f"{float(s['storage_utilization']):.2%}"
                    if s.get("storage_utilization") is not None
                    else "—"
                ),
            }

        cmp = pd.DataFrame([_scheme_product_row(s) for s in st.session_state.schemes])
        st.dataframe(cmp, use_container_width=True, hide_index=True)
        with st.expander("已保存方案 · 产品与车位", expanded=False):
            view_i = st.selectbox(
                "查看配置",
                options=list(range(len(st.session_state.schemes))),
                format_func=lambda i: st.session_state.schemes[i]["name"],
                key="scheme_product_view_sel",
            )
            saved_pc = st.session_state.schemes[int(view_i)].get("product_config")
            if saved_pc:
                render_scheme_product_config_panel(saved_pc)
            else:
                st.caption("该方案为旧版保存，无产品配置快照。")
        sel = st.selectbox(
            "选择要删除的方案",
            options=list(range(len(st.session_state.schemes))),
            format_func=lambda i: st.session_state.schemes[i]["name"],
            key="scheme_delete_sel",
        )
        d1, d2 = st.columns(2)
        if d1.button("删除所选"):
...
def main() -> None:
    ...
    with col_left:
        if not st.session_state.site_basics_confirmed:
            ...
            st.markdown(
                '<p class="muted-hint">先完成 <strong>第一部分：场站资源配置方案</strong>，'
                "再进入 <strong>第二部分：施工成本</strong> 与 <strong>第三部分：运营与测算</strong>。"
